...
- `content.py` 定义发布快照与语义身份，`job_models.py` 统一任务状态及可执行操作。内容、摘要、版本通过 `Store.commit_content` 同事务提交，编辑必须带预期版本；接口返回预览和版本须取同一快照。禁止从残留 parsed.json 覆盖已提交内容。
- 发布确认必须绑定内容版本、标题、素材、应用及目标；复用结果按当前发布计划只读核验，未知写入不能通过新任务绕开。所有本地块和素材先通过 preflight 再创建远端对象。
