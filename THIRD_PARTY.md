...
实现和许可证记录日期：2026-09-21。完整 Python 依赖版本以 `uv.lock` 为准；模型权重不随项目提交。
...
| Beautiful Soup / Playwright | 公众号 HTML 解析与独立临时验证浏览器 | [Beautiful Soup](https://www.crummy.com/software/BeautifulSoup/) MIT；[Playwright](https://github.com/microsoft/playwright-python) Apache-2.0，浏览器附带其自身许可 |
| feishu-markdown | 参考富文本和块类型映射思路，保留 Python 发布与恢复实现 | [源仓库](https://github.com/huandu/feishu-markdown)，MIT；未直接移植源码 |
