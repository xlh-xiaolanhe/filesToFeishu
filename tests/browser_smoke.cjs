...
(async () => {
  ...
  try {
    ...
    const configured = await (await page.request.get(new URL("/api/health", page.url()).href)).json();
    await page.locator("#target").fill(configured.default_target);
    ...
    const openedId = await page.locator("#history").inputValue();
    await page.reload();
    await page.locator(`#history option[value="${openedId}"]`).waitFor({state: "attached"});
    await page.locator("#history").selectOption(openedId);
    await page.locator("#target").fill(configured.default_target);
