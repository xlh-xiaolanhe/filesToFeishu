...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator(".code-preview").evaluate(node => node.firstElementChild.tagName), "PRE");
    assert.equal(await page.getByText("校对识别结果", { exact: true }).isVisible(), true);
    ...
    assert.deepEqual(await page.locator(".code-preview").evaluateAll(nodes => nodes.map(node => node.firstElementChild.tagName)), ["PRE", "PRE"]);
